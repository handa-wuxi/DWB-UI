...
import { useTabsViewStore, useProjectSettingStore, useAsyncRouteStore } from '@/store';
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const getChangeStyle = computed(() => {
      ...
      let lenNum = '';
      if (navMode === 'horizontal' || !isMixMenuNoneSub.value) {
        lenNum = '0px';
      } else if (collapsed) {
        lenNum = `${minMenuWidth}px`;
      } else {
        lenNum = `${menuWidth}px`;
      }
    ...
    for (let i = 0; i < cacheRoutes.length; i += 1) {
      const cacheRoute = cacheRoutes[i];
    ...
    const whiteList: string[] = [
      GlobalStoreEnum.BaseLoginName,
      GlobalStoreEnum.RedirectName,
      GlobalStoreEnum.ErrorPageName,
    ];
    ...
    window.addEventListener('beforeunload', () => {
      storage.set(GlobalStoreEnum.TabRoutes, JSON.stringify(tabsList.value));
    });
    ...
    const closeAll = () => {
      ...
      router.replace(GlobalStoreEnum.BaseHome);
      updateNavScroll();
    };
